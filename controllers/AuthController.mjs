...
//Query the database to check if the email exists
  db.query(
    ...
    async (err, userResults) => {
      if (err) {
        return res.status(500).send("Error, el servidor no pudo buscar el correo electrónico", err);
      }
      if (userResults.length === 0) {
        return res.status(401).send("El correo electrónico ingresado no está registrado. Por favor, verifica y vuelve a intentarlo.");
      }
     ...
     //If the password matches, create the security token
      if (passwordMatch) {
        ...
        return res.status(401).send("Contraseña incorrecta. Por favor, verifica e inténtalo de nuevo.");
      }
    }
  );
};
