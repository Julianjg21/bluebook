      ...
      //Generate the password hash asynchronously
       ...
       //Responds with a status of 500 and an error message
        if (error.code === 'ER_DUP_ENTRY') {
            res.status(400).json({ error: "El correo electrónico ya se encuentra registrado. Por favor, usa otro." });
        } else {
            res.status(500).json({ error: "El servidor no pudo guardar los datos. Por favor, inténtalo de nuevo." });
        }
    }
};
