...
const verifyResetCodeMiddleware = (req, res, next) => {
  ...
  db.query("SELECT codeExpiration, resetCode FROM users WHERE email = ?", [email], (err, userResults) => {
    if (err) {
      console.log("Error en la consulta de verificación:", err);
      return res.status(500).json({ message: 'Error al verificar el código', error: err });
    }

    if (!userResults.length || userResults[0].resetCode !== code || Date.now() > userResults[0].codeExpiration) {
